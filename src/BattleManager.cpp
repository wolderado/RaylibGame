...
void BattleManager::ProcessAI(shared_ptr<Fighter> fighter) {
    ...
    if(fighter->CurrentAIState == AIState::FindTarget)
    {
        ...
        for (auto& pair : worldInstance->activeGameObjects)
        {
            ...
            if(fighter->GetTeam() == TEAM_ENEMY && (pair.second->GetTeam() == TEAM_PLAYER || pair.second->GetTeam() == TEAM_ALLY))
                targetTeam = true;
        ...
        {
            bool targetTeam = false;
            if(fighter->GetTeam() == TEAM_ENEMY && (rndObject->GetTeam() == TEAM_PLAYER || rndObject->GetTeam() == TEAM_ALLY))
                targetTeam = true;
